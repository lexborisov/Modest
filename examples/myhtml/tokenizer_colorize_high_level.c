...
#include "../../third_party/console-colors/console-colors.c"
...
void colorize_print(mycore_incoming_buffer_t *inc_buf, size_t begin, size_t length, cc_color_t color)
{
    if(length) {
        ...
        cc_fprintf(color, stdout, "%.*s", (int)length, &between_data[between_begin]);
    }
}
    ...
    while(attr)
    {
        ...
        if(key_pos.length)
        {
            ...
            if(last_pos < key_pos.begin)
                colorize_print(inc_buf, last_pos, (key_pos.begin - last_pos), CC_FG_RED);

            /* print <div [key]=value> */
            colorize_print(inc_buf, key_pos.begin, key_pos.length, CC_FG_YELLOW);
            ...
            if(value_pos.length && last_pos < value_pos.begin)
                colorize_print(inc_buf, last_pos, (value_pos.begin - last_pos), CC_FG_RED);
        }
        ...
        if(value_pos.length)
        {
            ...
            if(key_pos.length) {
                ...
                colorize_print(inc_buf, between_begin, (value_pos.begin - between_begin), CC_FG_RED);
            }

            /* print <div key=[value]> */
            colorize_print(inc_buf, value_pos.begin, value_pos.length, CC_FG_BLUE);
...
void * colorize_callback_before_token_done(myhtml_tree_t* tree, myhtml_token_node_t* token, void* ctx)
{
    ...
    switch (myhtml_token_node_tag_id(token)) {
        case MyHTML_TAG__DOCTYPE: {
            /* print [<!DOCTYPE] */
            colorize_print(inc_buf, token_element_pos.begin, (token_pos.begin - token_element_pos.begin), CC_FG_GRAY);

            colorize_print(inc_buf, token_pos.begin, token_pos.length, CC_FG_GRAY);

            /* print [>] */
            colorize_print(inc_buf, last_pos, ((token_element_pos.begin + token_element_pos.length) - last_pos), CC_FG_GRAY);
            break;
        }
        case MyHTML_TAG__TEXT: {
            colorize_print(inc_buf, token_pos.begin, token_pos.length, CC_FG_WHITE);
            break;
        }
        case MyHTML_TAG__COMMENT: {
            /* print [<!--] */
            colorize_print(inc_buf, token_element_pos.begin, (token_pos.begin - token_element_pos.begin), CC_FG_GREEN);

            colorize_print(inc_buf, token_pos.begin, token_pos.length, CC_FG_GREEN);

            /* print [-->] */
            colorize_print(inc_buf, last_pos, ((token_element_pos.begin + token_element_pos.length) - last_pos), CC_FG_GREEN);
            break;
        }
        default: {
            /* print [<]div> */
            colorize_print(inc_buf, token_element_pos.begin, (token_pos.begin - token_element_pos.begin), CC_FG_RED);

            /* print <[div]> */
            colorize_print(inc_buf, token_pos.begin, token_pos.length, CC_FG_RED);
            ...
            /* print <div[>] */
            colorize_print(inc_buf, last_pos, ((token_element_pos.begin + token_element_pos.length) - last_pos), CC_FG_RED);
